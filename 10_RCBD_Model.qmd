...
\begin{align*}
    ...
    \alpha_i &= \text{effect of the } i^{th} \text{ treatment} \\
    \beta_j &= \text{effect of the } j^{th} \text{ block} \\
...
It also says that these effects are additive. Additivity means that the effect of the $i^{th}$ treatment on the response ($A_i$) is the same regardless of the block in which the treatment is used. Similarly, the effect of the $j^{th}$ block is the same ($B_j$) regardless of the treatment. The addiiton $\sum_{j=1}^{b} \beta_j = 0$ follows the same logic as explain before.

Let's fit this model to the Pygmalion data. For the pygmalion experiment, the reserachers compared control to pygmalion treatment so we have $a=2$ treatments. The number of blocks, $b$, was 10. In R, on the right-hand-side of the formula, we have the treatment factor + blocking factor; their effects are assumed to be additive. The code looks exactly the same as before, except we **add** the Company (blocking) variable.
...
First we see the grand mean of 73.31 and the treatment means. Then, we have ten block means, these are the mean scores within each block. Lastly, we see the standard errors for the differences of means.
...
What does the ANOVA table look like for the Pygmalion data? Again, we use the `summary` function to obtain the table.
...
Let's first look at the degrees of freedom. We had $a \times b = 2 \times 10 = 20$ experimental units, so we should have $a \times b - 1$ total degrees of freedom. The treatment degrees of freedom are $a-1 = 1$ and similarity, there are $b-1$ degrees of freedom of the block effect. That leaves $(a-1)(b-1) = 1 \times 9$ degrees of freedom for the error term. It is always a good idea to check that the df's match with what you expected them to be. One serious error that happens easily is one of the factor is fitted as continuous covariate because the levels were labelled using numbers. Hence why we converted the categorical variables (Treat and Company) to factors.
