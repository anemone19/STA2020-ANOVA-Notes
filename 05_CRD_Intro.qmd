...
```{r plot, label= fig-socialboxplot, fig.cap="Boxplots of Post treatment scores by group."}
...
The assumption is that the errors are independent. While we can check for certain types of dependence in the residuals (estimates of errors) after fitting the ANOVA (as we will see later), dependence among observations generally results in dependent residuals. Therefore, before fitting any models, we examine the observations and the experimental design to identify potential violations of independence.
...
A statistical procedure is said to be robust to departures from a assumption if it is valid even when the assumption is not met. The robustness of ANOVA is as follows:

1.  The assumpiton of normality is not crucial. Only severe departures from normality such long-tailed distributions or skewed distributions when sample sizes are unqeual and small are particurlarly problematic.
...
4.  ANOVA is not very resistant to severly outlying observations either.
...
-   Checked the dataset for correct structure (120 observations, treatment groups as factors).\
-   Inspected summary statistics and visualized distributions (boxplots, histograms, Q-Q plots).

For ANOVA, the following assumptions were examined:

1.  **Outliers**: Check via boxplots.\
