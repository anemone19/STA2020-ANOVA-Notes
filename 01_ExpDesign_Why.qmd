...
Experimental studies are extremely important in research and in practice. They are almost the only way in which one can control all factors to such an extent as to eliminate any other possible explanation for a change in a response other than the variable actively manipulated. In this section, we only consider experimental studies and those which aim to compare the effects of a number of **treatments** (comparative experiments).
...
Experimental studies and their design are fundamental to understanding the world around us, allowing us to further knowledge and test theories. So lets define them more rigorously. We'll start by introducing some terminology.
...
4. We consider comparative experiments where the aim is to compare the mean response between treatments. 
