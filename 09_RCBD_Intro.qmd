...
If private school students perform better, we cannot conclude school type caused the difference due to potential confounders such as socioeconocmic background or teacher quality.
...
-   Subject: person, plant, bussinesses, genotype.
-   Demographic groups: age, gender, income or education level, consumer behaviour segments.\
...
Back in 1990, one researcher in this field, noticed that experiments like these might involve something that is called interpersonal contrasts. When some individuals are singled out for high expectations, other might feel neglected. This could potentially skew the results by making the others look good even though it was just the rest that perfomred poorly. The researcher wanted to conduct an experiment to test the Pygmalion effect without interpersonal contrasts.

The reasearcher achieved this by applying the high expectation to an entire group and not selected individuals within a group. Let's have a look the exact experiment!
...
Also, as before, I've modifed the example slightly. In the original study, there three platoons per compancy with two serving as control and one company only had two. So we have simplied the design so that it is balanced.
:::
...
pyg_wide <- reshape(pyg_data, 
                     idvar = "Company", 
                     timevar = "Treat", 
                     direction = "wide")

colnames(pyg_wide) <- c("Company","Pygmalion","Control")
rownames(pyg_wide) <- NULL


knitr::kable(pyg_wide, "pipe") %>%
  kable_styling("striped", full_width = F) 
```
