...
To answer the first question, our contrast should compare the difference between the audio-visual and audio-only. So we want to combine the treatments containing each level of Content.Type and compare these two: 

$$(\mu_{1AV} + \mu_{2AV}) - (\mu_{1AO} + \mu_{2AO})$$ 


Now we are comparing groups of means. The first group contains the means for all treatments that included Audio-Visual level and the second contains the Audio-Only level. The coefficients of the contrast sum to zero:  

$$
\begin{aligned}
&(\mu_{AV1} + \mu_{AV2}) - (\mu_{AO1} + \mu_{AO2}) \\
&(1) \mu_{AV1} + (1) \mu_{AV2} + (-1) \mu_{AO1} + (-1) \mu_{AO2} \\ 
& 1 + 1 -1 -1 = 0
\end{aligned}
$$
This is a linear combination of our model parameters. What does the contrast and coefficients look like for the second question? To answer whether playback speed decreased comprehension, we want to compare the treatments with 1x speed to those with 2x times speed: 

$$(\mu_{1AV} + \mu_{1AO}) - (\mu_{2AO} + \mu_{2AV})$$
The coefficients sum to zero as before. This might be confusing but we are simply grouping treatments together and comparing them. To do this in R, we first need to refit our model using the function `lm` as we did before when we performed contrasts and then we extract the treatment means with a new function `emmeans` from the package `emmeans`. 
...
emmeans(model_reg,~Content.Type * Speed)
...
Let's start with the third question, this is the interaction. The contrast for the interaction should compare the *difference between audio-visual and audio-only in the two levels of playback speed*:
