...
We also need to know what the observational units are. The text states that at the end of therapy, each student completed an evaluation to determine their level of test anxiety. So the response, test anxiety, was measured on the student level which means students are the observational units. In the first scenario, the students are both the experimental units and observational units. But this would not be the case if groups are the experimental unit. In that case, the observational units would still be the students, but the experimental units would be the groups of students. In this case, we would have to take the mean of the test anxiety scores for each group to end up with one response value per experimental unit (group).
