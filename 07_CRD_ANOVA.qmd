...
This division of the total sums of squares is typically summarised in an analysis of variance table. The first column contains the "source" of the varibility with the first entry (the order is not important, although this is the typical order) representing the between treatment variability (explained variation), second the error (unexplained variation) and lastly the total variation. The second column gives the sums of squares of each source. The third column contains the degrees of freedom.
...
Read it carefully. The table says: "Entries in the table give F! values, where ! is the area or probability in the upper tail of the F distribution. For example, with four numerator degrees of freedom, eight denominator degrees of freedom, and a.05 area in the upper tail, F.05 = 3.84." This is important, not all tables look like this. See if you can find the F-value mentioned.

The numeratir df is in the column and the denominator df is in the row. In the row dimension are different probability values as well. To find an F-value, locate the df in the column and row. Can you find the following:
...
This is how we find critical values of F-distributions. If you are asked to compare a test statistic with a critical value at a specific signfiicance level, you will find the value with a table like this. To find the critical values in R, we use the `fq` function:
...
In words we say that the average academic performance of students did not different across the treatments (levels of socal media multitasking).

And the alternate hypothesis is the opposite of that:

$$H_A: \text{At least one } \mu_i \text{ is different.}$$ At least one of the social media multitasking treatments resulted in a different mean academic performance, they are not all equal.
...
The first two you can answer with the degrees of freedom and the third is answered by conducting the hypothesis test. With three treatments, we have 2 treatment degrees of freedom. We had 40 studnets per group, the sample size is then 120 which means there are 117 degrees of freedom for the residuals. The treatment MS (5488) wis much larger that the MSE (200). This leads to an F-ratio of 27.42 with a p-value of $1.72\times e^{-10}$ (thats extremely small). We have strong evidence that the treatments did produce different academic performances across students. At least one treatment resulted in a different mean academic performance. In a report, you would write:
...
[^07_crd_anova-4]: The same concepts apply to linear regression models.

We call the function `plot` on our model object. For our purposes we are only going to look at two of the plots and we inspect them one by one by specifyng the plot number with the argument `which`:
...
This is a plot of the residuals (obs - fitted) against the fitted values and we are hoping to see no patterns. We have three lines, one for each treatment group and we want to check that our residuals are centred around zero and have constant variance across the groups. [^07_crd_anova-5]

[^07_crd_anova-5]: Remember we assumed $e_{ij} \sim N(0,\sigma^2)$ and residuals are estimates of the errors.
...
The second plot is a Q-Q plot which we have seen before when we checked the assumptions before model fitting. Now, we plot the standardised residuals aganst the theoretical quantiles of a standard normal distribution. We are looking for the same pattern as before, that the points fall close to the dotted line. As usual, there many be soem deviations at the tails but for the most part, there are no serious problems with this plot. If there is some doubt, we can also look at a histogram of the residuals:
...
The assumption of independent errors is mostly checked before model fitting and with considering the experimental design. If we suspected autcorrelated residuals, we could plot the residuals againt order:
...
That's a alot. So let's sumamrise what we did in this chapter:

We introduced Analysis of Variance (ANOVA), which is fundamentally a regression model with categorical variables but parameterized differently. ANOVA allows us to partition total variance into between-treatment and within-treatment variability, helping us determine whether differences in the response variable are due to the treatments.
...
The F-test determines statistical significance, and its p-value is derived from the F-distribution. A small p-value suggests strong evidence against the null hypothesis ($H_0$), indicating at least one group mean differs. The ANOVA table summarizes these calculations, including sums of squares (SS), degrees of freedom (df), mean squares (MS), and the F-statistic.

Applying ANOVA to real experimental data, we analyzed the impact of social media multitasking on student performance. With three treatment groups (control, SMS, Facebook), we found a statistically significant effect ($F_{2,117} = 27.42$, $p<0.001$), confirming that at least one treatment influenced academic performance. However, ANOVA does not specify which groups differ—this requires post-hoc tests.
...
These diagnostic checks confirmed that our ANOVA model met assumptions, supporting the validity of our conclusions.
